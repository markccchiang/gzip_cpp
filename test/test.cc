...
TEST(gzip, compress) {
    ...
    g_compressed_data = comp.Process(SOURCE_STRING, strlen(SOURCE_STRING) + 1, true);
...
TEST(gzip, decompress) {
    ...
    for (const gzip::Data& data : g_compressed_data) {
        ...
        EXPECT_TRUE(succ);
        std::copy(this_out_data_list.begin(), this_out_data_list.end(), std::back_inserter(out_data_list));
    }
...
TEST(gzip, large_compress) {
    ...
    for (int i = 0; i < NUM_OF_COMPRESS_TIMES; ++i) {
        gzip::DataList compressed_data_list = comp.Process(g_random_data, i == NUM_OF_COMPRESS_TIMES - 1);
        std::copy(compressed_data_list.begin(), compressed_data_list.end(), std::back_inserter(g_random_compressed_data_list));
    }
}
