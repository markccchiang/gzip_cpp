...
Comp::Comp(Level level, bool gzip_header) : level_(level) {
    ...
    int ret = deflateInit2(&zs_, static_cast<int>(level_), Z_DEFLATED, windowBits, 8, Z_DEFAULT_STRATEGY);
    init_ok_ = ret == Z_OK;
}
...
std::tuple<bool, DataList> Decomp::Process(const Data& compressed_data) {
    ...
    zs_.next_in = reinterpret_cast<unsigned char*>(const_cast<char*>(compressed_data->ptr));
    int ret;
    do {
